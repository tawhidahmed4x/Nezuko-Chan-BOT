...
async function startBot() {
    ...
    try {
        if (process.env.APPSTATE) {
            ...
            console.log(colors.hex("#00FF00")(" ✅ [SYSTEM] Using Render Environment AppState."));
        } else {
    ...
    } catch (e) { log.err("LOGIN", "AppState format error!"); }
    ...
    login({ appState }, { forceLogin: true, logLevel: "silent" }, async (err, api) => {
        if (err) return log.err("LOGIN", "Login Failed! Check cookie.");
        ...
        try {
            // PATH: সরাসরি bot/login/loadData.js
